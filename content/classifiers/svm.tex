...
\end{equation}
where \( \boldsymbol{e} = [1,1, ...,1]^{\top} \), \( \boldsymbol{o} = [0,0, ...,0]^{\top} \), \( \boldsymbol{X} = [\boldsymbol{x_1},\boldsymbol{x_2}, ...,\boldsymbol{x_n}] \), \( \boldsymbol{y} = [y_1,y_2, ...,y_n]^{\top} \), \( Y = \text{diag}(\boldsymbol{y}) \), \( \boldsymbol{B_e} = [\boldsymbol{y}^T] \) and \( \boldsymbol{K}\coloneqq\boldsymbol{X}^T\boldsymbol{X} \) is the Gram matrix which is symmetric positive semi-definite (SPSD)\cite{Aeta2018}. The Hessian matrix in \eqref{eq:svm:soft_margin_dual}
\begin{equation}
...
Grid search is an exhaustive searching through a user specified subset of parameters. The classifier is trained with each combination of the subset. The combination, which yield the best result is then selected. By the best result, we mean the highest score of a user specified metric.
...
As a randomly selected fold might not represent the classes in the same ratio as the whole set, a stratified cross-validation is used. The random folds are selected in a way, where the ratio of the classes is roughly equal to the ratio of classes in the whole dataset.
